...
| **P0-19（v3.5 新增，调度真值一致性）** | **Dispatch Decision 不是观测标签，而是必须被真实 Executor 落地的执行契约。系统记录的 `effective_mode/worker_count/execution overlap` 必须来自真实运行事实；若资源、能力或入口限制导致并行无法落地，必须在执行前显式降级并记录原因，禁止出现“记录为 PARALLEL/DECOMPOSE，但实际仍由单 Worker 串行执行”的伪调度。** | v3.5 |
...
════════════════════ Reliability Boundary ════════════════════
                    ...
                    └──────────┬──────────┘
                               │
                    ┌──────────▼──────────┐
                    │  DISPATCH EXECUTOR   │  requested_mode → effective_mode
                    │ + CONTROLLER/POOL    │  Lease + Fencing + Resource Ceiling
                    └──────────┬──────────┘
                               │
                    ┌──────────▼──────────┐
...
| **Execution Surface Attestation（新）** | 会话/入口记录 entrypoint、bundle/commit、Ingress/Pipeline/Scheduler 是否真实绑定，解决“看起来在 PPH 中、实际绕过 PPH” | v3.5 |
| **Dispatch Execution Contract（新）** | 区分 requested/effective dispatch mode；把 Policy 选择与真实 WorkerPool 执行绑定，落地 P0-19 | v3.5 |
...
| **Production Dispatch Executor（新）** | 将 Ready leaf Tasks 交给 WorkerPool，执行真实 fan-out/fan-in；资源压力降级串行时持久化真实 effective mode | v3.5 |
| **Domain Backpressure Hook（新）** | 对网络/Provider 类工具按 domain/provider 做并发与 Retry-After 管理，禁止用主 Agent `sleep` 人工排队 | v3.5 |
...
## 47. Dispatch → Execution 真实闭环与 Research Fan-out（v3.5 新增）

> **问题来源**：2026-09-19 对一条真实 PPH session（约 31 分 42 秒）做执行轨迹审计后发现：66 次工具调用里 65 次为顶层 `bash`，没有任何 Worker/Agent 调度事件；显式 `sleep` 累计 321 秒；末轮模型调用已携带约 72.5K token 缓存上下文；同时出现过把宿主环境变量直接打印进 transcript 的敏感信息暴露。仓库里的 WorkerPool、Lease/Fencing、resource ceiling 和 Phase 13 Multi-Worker 场景本身均已存在并通过专项验证，因此本次缺口不是“没有 Multi-Worker 能力”，而是**Dispatch Policy、真实 Executor、用户执行入口与观测真值之间没有形成生产闭环**。

### 47.1 Dispatch Decision 必须成为可执行契约（P0-19）

从 v3.5 起，`DispatchDecision.mode` 只表示**请求的执行策略**，不能直接作为“实际发生了什么”的事实。持久化 Dispatch 必须区分：

```yaml
dispatch_execution:
  requested_mode: single|decompose|parallel|batch
  effective_mode: single|decompose|parallel|batch
  executor_kind: direct_worker|worker_pool|batch_executor|decomposer
  planned_worker_count:
  effective_worker_count:
  degrade_reason: null|string
  overlap_proof_ref: null|artifact_ref
```

规则：

1. `requested_mode=parallel` 只有在真实 Executor 成功获得至少 2 个独立 Worker Instance 且执行时间窗口存在重叠时，`effective_mode` 才能保持 `parallel`。
2. 资源压力、Worker 不足、Provider backpressure 等导致只能串行时，必须在开始业务执行前把 `effective_mode` 降为 `single`，记录 `degrade_reason`；不得继续保留“parallel”标签。
3. `requested_mode=decompose` 不得直接把父 Task 交给一个 Worker 执行后声称“已拆分”。必须先通过原子 Graph Mutation 物化子 Task Contract，再由调度器处理 READY 叶子节点。
4. `requested_mode=parallel` 的执行对象是**至少两个已独立可验收的 READY leaf Task**。单个 Task 只有在先完成 decomposition/compilation、生成独立 leaf 后才具备并行语义。
5. Trace/Decision Record 中的 `graph_width/peak_active_workers/overlap` 一律从真实 Worker 生命周期事件派生，禁止由 Dispatch Policy 预填。

这条规则正式落地 **P0-19：调度真值一致性**。

### 47.2 Production Dispatch Executor

v3.5 在既有 `Dispatch Policy` 与 `Worker Adapter` 之间明确增加一个生产执行层，仍属于 Control/Execution 两域边界内，不新增第六域：
...
Dispatch Policy
      ↓ requested_mode
Dispatch Executor
  ├─ SINGLE  → Direct Worker Adapter
  ├─ DECOMPOSE → Atomic Graph Mutation → Ready Queue
  ├─ PARALLEL → WorkerPool.executeResourceAwareBatch(READY leaf tasks)
  └─ BATCH → Batch Executor → per-Task Result/Verification
      ↓ effective_mode + runtime facts
Persistent Dispatch Record / Trace
```

**关键边界**：`PersonalPiPipeline` 仍负责单 Task 的可靠执行闭环（Run/Evidence/Verification/Acceptance）；跨多个 READY Task 的并行 wave 由 Dispatch Executor 编排。不能为了“接上 Multi-Worker”把一个 Pipeline 调用内部变成多个互相污染的 Worker transcript，也不能绕过每个 leaf Task 自己的 Verification。

### 47.3 Independent Work Unit：研究/多实体任务也必须能并行

此前 `parallelism` 的确定性提示主要依赖 `scope.files` 数量，容易漏掉“没有多个代码文件、但存在多个天然独立研究对象”的任务。v3.5 将**Independent Work Unit** 定义为通用并行判定输入，不把它做成新的真相源：最终仍必须物化为标准 Task Contract。

Assessment 可追加以下非权威建议字段：
...
parallel_plan_hint:
  independent_units:
    - key:
      ...
      output_schema_ref:
  shared_context_refs: []
  fan_in_required: true|false
```

确定性规则优先识别以下模式：

- 用户显式给出多个互不依赖的实体/厂商/URL/数据集，并要求同一套字段；
- 多个只读分析目标不存在写冲突；
- 各子结果能独立产生 Evidence/Receipt，并由一个下游 synthesis Task 消费；
- 任一单位失败不会使其他单位的事实结果失效。

例如“分别调研 10 家模型厂商的 MAU/DAU/token，再汇总成本”应先生成按厂商或来源簇拆分的 leaf Tasks，再 fan-out；不应让一个 Master 在同一长会话里串行执行几十次搜索。

### 47.4 Execution Surface Attestation：证明这次请求真的走了 PPH

Ingress Binding Verification 解决“测试时有没有经过 Pipeline”，但真实 session 还需要能在事后直接回答“当时到底走的是哪个入口/哪个 bundle/是否绑定 scheduler”。从 v3.5 起，面向用户的会话头或等价只读元数据必须记录：
...
- 元数据只用于 attestation/审计，不进入 Prompt View，不作为任务完成真相源；
- `ingress_bound=false` 或 `pipeline_bound=false` 的入口不能执行超出 P0-17 兜底边界的实质性多步骤任务；
- 用户真实入口 E2E 必须断言 session attestation 与 Persistent State/Dispatch Trace 相互一致；
- 发布包和源码开发入口必须能区分，避免“当前源码已修复，但用户实际运行的是旧 bundle/另一入口”无法定位。

### 47.5 Research/Network Backpressure：等待属于调度器，不属于 Master

本次 session 中 35 个工具调用包含显式 `sleep`，累计 321 秒。v3.5 明确：**Provider/domain 限流、Retry-After、冷却窗口属于执行基础设施的 backpressure 状态，不应通过 Master 在 shell 中 `sleep` 来实现**。

通用规则：

1. 网络检索/HTTP/API 类工具向调度层报告 `domain/provider/status/retry_after_ms` 等结构化状态。
2. 同一 domain/provider 的并发由 T12.6 的 Backpressure/Circuit Breaker 统一约束；等待中的 Task 释放 Master 推理占用，其他 READY Task 可继续执行。
3. 有专用 Search/Fetch/Browser 工具时优先使用专用工具；需要退回 Bash/curl 时仍必须经过 Command Risk + Tool Gateway + domain backpressure hook。
4. `sleep` 只能作为底层 backoff 实现细节，不能作为模型自由生成的“排队策略”。
5. PPH 核心只提供通用 hook/状态与预算，不内置任何单一搜索引擎或 Provider 的业务 API，继续遵守第 46 节能力边界。

### 47.6 Secret-safe Tool Boundary

T10.3 已规定 credentials 默认拒绝，v3.5 将这条原则落实到“工具输出也不能把宿主凭据带进 transcript/Artifact 摘要”这一层：

- `credentials=deny` 时，`env/printenv/set` 等全量环境枚举和可等价导出 secret 的 shell 形式默认 `danger→block`；确需诊断时使用只返回批准 key 的专用能力。
- Tool Gateway 的敏感信息检测/脱敏必须识别 secret-like **键名**（例如 `*_TOKEN/*_API_KEY/*_SECRET/*_PASSWORD/*_COOKIE/*_AUTHORIZATION`），而不是只匹配值前面恰好写成 `token=` 的少数格式。
- 原始归档若包含敏感信息，只允许按既有 sensitive artifact 规则保存/访问；Prompt View、Receipt、Evidence summary 一律只看到脱敏版本。
- 发现 transcript 已持久化真实凭据时，视为安全事件：停止继续传播该值，并要求凭据轮换；不得仅依赖后续 compaction “把它忘掉”。

### 47.7 Fan-in 只消费 Receipt/Evidence，研究事实必须有 provenance

v3.3 已冻结 Receipt-only Fresh Context。v3.5 将其用于并行研究：fan-in/synthesis Task 不读取每个 Worker 的完整搜索 transcript，只读取结构化 Receipt、Evidence ref 和必要 Artifact ref。

对“数字/事实/来源”类交付，Evidence 允许附带通用 provenance：

```yaml
source_provenance:
  source_ref:
  source_type: official|filing|media|third_party|derived
  published_at:
  observed_at:
  metric:
  value:
...
`derived` 数值必须保存公式或输入 Evidence refs；来源冲突应并存并显式标记，不能在 synthesis 阶段仅凭模型记忆挑一个数。该结构是 Evidence 的可选通用元数据，不创建新的事实真相源。

### 47.8 WorkerPool 两项实现修正

1. **Warm-up 可并行**：互不依赖的 Worker Instance 启动不应在 `warmAllAsync()` 内人为串行；并发启动仍受 `resource_ceiling.max_parallel_workers` 约束。
2. **Role 并发计数按 distinct role 计算**：`max_concurrent_roles` 限制的是角色数量，不等于 `active_workers`。同一 researcher Role 的 4 个 Worker 应计为 1 个 concurrent role；多个 Role 同时执行才分别计数。WorkerPool/Dispatch Executor 必须从 Task/Role binding 提供真实 role id，不能用 Worker 数代替。

### 47.9 v3.5 回归基准：把本次 session 变成真实 Eval

固定一组“多实体只读研究”任务作为 v3.5 benchmark，在同预算、同来源范围、同工具权限下比较：

| 指标 | Single baseline | Multi-Worker |
|---|---|---|
| verified success rate | 必记 | 必记 |
| wall time | 必记 | 必记 |
| tool calls / model calls | 必记 | 必记 |
| explicit model-generated sleep | 必记，目标趋近 0 | **必须为 0** |
| peak active workers / overlap | 1 | ≥2 且有 timestamp 证据 |
| Master Prompt View tokens | 必记 | 必须显著低于“把全部搜索历史塞给 Master”的实现 |
| provenance coverage | 必记 | 每个最终关键事实均有 Evidence ref |
| credential exposure | 0 | **必须为 0** |

Multi-Worker 的目标不是“Worker 越多越好”。如果相同成功率下墙钟时间/上下文/成本没有改善，Policy Calibration 必须保留这个负结果，后续降低 fan-out；不得因为系统支持多 Worker 就强制所有任务并行。

### 47.10 v3.5 Stage Gate

v3.5 优化只有同时满足以下条件才算完成：

1. 真实 `pph` 用户入口产生可核验的 Execution Surface Attestation，证明 Ingress/Pipeline/Scheduler 已绑定。
2. `requested_mode` 与 `effective_mode` 分离持久化；任何降级都有原因；不存在“PARALLEL 标签 + 单 Worker 实际执行”。
3. 至少一个多实体只读任务从真实入口被物化为 ≥2 个独立 leaf Task，并通过 WorkerPool 产生真实 execution overlap。
4. fan-in 只消费 Receipt/Evidence/Artifact refs，不继承 Worker 完整 transcript。
5. domain/provider backpressure 不再由 Master 自由生成 `sleep` 控制。
6. `credentials=deny` 场景下环境枚举被阻断，secret-like ToolResult 能可靠脱敏。
7. Worker warm-up 并发与 distinct-role coordination 计数测试通过。
8. 同任务 Single vs Multi benchmark 有完整数据；若 Multi 没有收益，如实保留，不以功能存在替代效率证据。
...
| **v3.5** | **2026-09-19** | **用户+ChatGPT 评审确认** | **流程变更 + 调度闭环优化（真实 session 驱动）** | **Control/Execution/Trace/Security/Eval + 文件管理方式** | **保留“latest 持续更新 + 实现前完整快照”的双轨流程；新增 P0-19 调度真值一致性；新增 Dispatch Execution Contract（requested/effective mode）、Production Dispatch Executor、Independent Work Unit、多实体 research fan-out、Execution Surface Attestation、domain/provider backpressure hook、Secret-safe Tool Boundary、研究 Evidence provenance；修正 Worker warm-up 并发与 distinct-role 计数；把 2026-09-19 单 Agent 研究 session 固化为 Single-vs-Multi 回归基准。** |

> **当前状态**：五域内核边界仍未被突破；v3.5 的变化集中在“已有调度能力如何成为真实用户入口的执行事实”，不新增第六域。后续实现必须先从本 `latest` 打完整 v3.5 快照，并在整个实现周期只读该快照。
