...
### T7.0-A Execution Surface Attestation（v3.5 新增）
- **优先级**：P0
- **需求**：真实用户 session 必须能直接证明本次请求到底从哪个 entrypoint/bundle/commit 进入，Ingress/Pipeline/Scheduler 是否真实绑定，避免再次出现“源码里已经接上 PPH，但实际运行 session 仍走另一条 legacy/旧 bundle 路径”而事后只能靠行为猜测。
- **数据来源**：架构文档第 47.4 节；2026-09-19 真实 session 审计。
- **权限**：只读读取当前入口标识、Git commit 与 bundle digest；不得因 attestation 增加业务写权限。
- **实现方案**：
  1. Session header 或等价只读元数据记录 `entrypoint/pph_commit/bundle_sha256/ingress_bound/pipeline_bound/scheduler_enabled/scheduler_kind`。
  2. 这些字段不进入 Prompt View，不参与业务完成判定，只用于审计。
  3. `ingress_bound=false` 或 `pipeline_bound=false` 时，执行入口只能走 T2.0-B 的 Fail-Closed 兜底，不得继续执行实质性多步骤任务。
  4. 真实入口 E2E 同时检查 session attestation、Persistent State 与 Dispatch Trace 三者一致。
- **失败恢复**：无法取得 commit/bundle digest → 明确记录 `unknown` 并阻止把该 session 当成“已证明 governed execution”的 Stage Gate 证据；不得伪造或从配置猜值。
- **验证方法**：分别从正式 bundle 和源码开发入口启动一次只读任务，验证 attestation 能区分两条 execution surface；故意去掉 ingress factory 时验证 session 被标记未绑定并 Fail-Closed。
- **Exit**：所有面向用户的正式执行入口都能产生可信 attestation，且不存在“未绑定却继续长任务”的路径。
...
### T2.3-B Dispatch Execution Contract（v3.5 新增，落地 P0-19）
- **优先级**：P0
- **需求**：Dispatch Policy 的输出必须与真实执行事实分离记录，新增/补齐 `requested_mode/effective_mode/executor_kind/planned_worker_count/effective_worker_count/degrade_reason/overlap_proof_ref`；禁止把 Policy 选择直接当成“已经执行”的事实。
- **数据来源**：架构文档第 47.1 节；当前实现中 `createDispatchDecision()` 可返回 `PARALLEL/DECOMPOSE`，但 Pipeline 仍可能只调用单个 `request.worker.execute()` 的真实缺口。
- **权限**：Persistent State/Trace 写权限（沿用现有 Dispatch/Decision Record 权限）。
- **实现方案**：
  1. `DispatchDecision.mode` 作为 `requested_mode`；Executor 接管后才确定 `effective_mode`。
  2. 资源压力/Worker 不足/Provider backpressure 导致串行时，在业务执行前持久化降级原因。
  3. `effective_mode=parallel` 必须由至少 2 个真实 Worker Instance 与 execution overlap 证据支撑；Trace 的 `peak_active_workers/graph_width` 从运行事实派生，Policy 不得预填。
  4. `requested_mode=decompose` 必须先原子物化子 Task Contract，再进入后续 Ready/Dispatch，不允许父 Task 单 Worker 直接执行并仍记录为 decompose。
- **失败恢复**：Executor 无法满足 requested mode → 显式降级为 `single` 或 `BLOCKED`，保留 requested/effective 差异；不得静默继续。
- **验证方法**：构造 Policy 返回 `PARALLEL` 但只有一个 Worker 可用的场景，验证 persisted Dispatch 为 `requested=parallel/effective=single` 且带 `degrade_reason`；再用两个真实 Worker 验证 `effective=parallel` 且 overlap ref 存在。
- **Exit**：代码库中不存在“记录为 PARALLEL/DECOMPOSE，但执行事实仍是未声明单 Worker”的路径。

### T2.3-C Independent Work Unit / Multi-Entity Parallel Hint（v3.5 新增）
- **优先级**：P1
- **需求**：Task Assessment 不再只依赖 `scope.files` 数量判断并行性；对多实体只读研究、多个 URL/数据集、多个互不写冲突的分析目标，能确定性地产出 `parallel_plan_hint.independent_units[]`，随后由 Decomposer 将其物化为标准 leaf Task Contract。
- **数据来源**：架构文档第 47.3 节；2026-09-19 多厂商模型数据调研 session。
- **权限**：无新增权限；只读取用户 Requirement/Task 描述与 scope。
- **实现方案**：
  1. 扩展 Assessment suggestion/output 的非权威字段：`independent_units[{key,objective,source_scope,output_schema_ref}]`、`shared_context_refs`、`fan_in_required`。
  2. 确定性规则至少识别“用户显式列出多个实体且要求同一组字段”“多个只读目标无依赖/无写冲突”两类模式。
  3. Hint 本身不能直接派 Worker；必须经 T1.3-A 原子 Graph Mutation 生成正式 Task Contract 后才可并行。
- **失败恢复**：无法证明独立性 → `parallelism=ineligible` 或退回 DECOMPOSE/人工确认，宁可串行也不制造错误并行。
- **验证方法**：用“10 家模型厂商分别采集同一组指标并汇总”样例，验证能生成 ≥2 个独立 units；用“两个目标都要改同一文件”样例验证不会误判为可并行。
- **Exit**：多实体研究样例不再因 `scope.files` 只有一个目录而退化为单 Agent 长会话。
...
### T4.1-B Source Provenance for Research Evidence（v3.5 新增）
- **优先级**：P1
- **需求**：数字/事实/来源类任务的 Evidence 可附带通用 `source_provenance`，至少包含 `source_ref/source_type/published_at/observed_at/metric/value/unit/period/population_scope/formula_ref/confidence`；派生数字必须能回指输入 Evidence 或公式。
- **数据来源**：架构文档第 47.7 节；本次研究 session 中“官方/第三方/估算数字在最终表格里没有逐条绑定原始来源”的可追溯性缺口。
- **权限**：无新增权限；复用 Evidence/Artifact Store。
- **实现方案**：
  1. Evidence schema 增加可选 provenance 元数据，不改变 Evidence 作为唯一验证输入的地位。
  2. `source_type=derived` 时 `formula_ref` 或输入 Evidence refs 至少存在一项。
  3. fan-in/synthesis 只能引用 provenance 完整的关键数字；冲突来源并存记录，不自动覆盖。
- **失败恢复**：关键事实缺 provenance → Verification 对对应验收项判 UNKNOWN/不完整，不允许 synthesis 把无来源数字升级成“已验证事实”。
- **验证方法**：构造两个来源互相冲突的同一指标与一个 derived 成本数字，验证三条 Evidence 均保留且 derived 数字能追溯公式/输入。
- **Exit**：研究类交付中的关键数字可从最终结果一路追到 source ref 与计算依据。
...
### T10.3-A Secret-safe Tool Boundary（v3.5 新增）
- **优先级**：P0
- **需求**：`credentials=deny` 时禁止通过通用 shell 枚举宿主全部环境变量；Tool Gateway 对 secret-like 键名做 key-aware 检测/脱敏，确保真实 token/key/password/cookie/authorization 不进入 Prompt View、Receipt 或 Evidence summary。
- **数据来源**：架构文档第 47.6 节；2026-09-19 真实 session 中环境变量输出把凭据写入 transcript 的安全事件。
- **权限**：无新增权限，是现有 credential deny 的执行层补强。
- **实现方案**：
  1. Command Risk 规则把 `env`/`printenv`/`set` 及等价“全量环境导出”形式在 `credentials=deny` 下判为 `danger→block`。
  2. Tool Gateway 敏感键匹配至少覆盖 `*_TOKEN/*_API_KEY/*_KEY/*_SECRET/*_PASSWORD/*_COOKIE/*_AUTHORIZATION`（大小写不敏感），值统一 `[REDACTED]`。
  3. 原始 Artifact 如被识别含敏感信息，沿用 sensitive artifact 访问规则；任何摘要/分页读取都先脱敏。
  4. 需要诊断环境时只能读取显式批准的单个 key，不提供“先全打印再 grep”的路径。
- **失败恢复**：检测到 transcript/summary 已包含真实 secret → 终止继续传播，记录安全事件并提示轮换凭据；后续 compaction 不视为修复手段。
- **验证方法**：构造 `CLOUDFLARE_API_TOKEN=secret-value`、`FOO_API_KEY=...` 等输出，验证所有摘要/分页均脱敏；验证 `credentials=deny` 下 `env`/`printenv` 被执行前拦截。
- **Exit**：secret 枚举拦截与 key-aware redaction 测试全过，真实 secret 暴露计数为 0。
...
### T11.5-A Real Ingress Single-vs-Multi Research Benchmark（v3.5 新增）
- **优先级**：P1，v3.5 Stage Gate 前置
- **需求**：把 2026-09-19 暴露问题的“多实体只读研究”模式固化为真实入口 benchmark；同预算、同来源范围、同工具权限比较 Single 与 Multi 的 verified success rate、wall time、model/tool calls、Prompt View tokens、显式模型 `sleep`、provenance coverage、credential exposure、peak active workers 与 overlap。
- **数据来源**：架构文档第 47.9 节；T11.5 既有 baseline 机制。
- **权限**：与 benchmark Task Contract 一致；默认 credentials deny。
- **实现方案**：
  1. 用固定 fixture 定义 ≥4 个独立研究实体和统一输出字段，不把期望答案放进 Worker Context。
  2. Single 模式强制 1 Worker；Multi 模式允许 Dispatch Executor fan-out，其他预算保持一致。
  3. 报告必须保留负结果；Multi 若更慢/更贵，不得修改指标或删掉样本。
- **失败恢复**：外部网络波动导致不可复现 → 优先使用本地可控 research fixture 验证调度结构，再另存真实网络观察值；结构验证与互联网质量分开。
- **验证方法**：至少跑一轮相同 fixture 的 Single/Multi，对比报告中 Multi 必须出现真实 overlap，模型生成的显式 `sleep` 必须为 0，credential exposure 必须为 0。
- **Exit**：报告可回答“Multi 是否真的更快/更省上下文/证据覆盖是否更好”，而不是只证明“能启动多个 Worker”。
...
### T12.4-A Production Dispatch Executor Binding（v3.5 新增）
- **优先级**：P0
- **需求**：把 T2.3-B 的 Dispatch Execution Contract 真正接到 WorkerPool；生产入口的 `PARALLEL`/`DECOMPOSE` 不再只停留在 Decision Record/专项脚本，而是进入统一 Scheduler/Executor 路径。
- **数据来源**：架构文档第 47.2 节；当前 `PersonalPiPipeline.execute()` 仍以单个 `request.worker` 为直接执行对象的缺口。
- **权限**：复用 WorkerPool/Lease/Persistent State 权限。
- **实现方案**：
  1. 新增/抽取 `DispatchExecutor`：SINGLE→Direct Worker；DECOMPOSE→Atomic Graph Mutation；PARALLEL→对 ≥2 READY leaf Tasks 调用 `WorkerPool.executeResourceAwareBatch()`；BATCH→既有 BatchResult 流程。
  2. Pipeline 保持单 leaf Task 的 Run/Evidence/Verification/Acceptance 责任；Executor 只编排 wave，不共享 Worker transcript。
  3. Executor 输出 `effective_mode/effective_worker_count/degrade_reason` 回写 Dispatch/Trace。
  4. resource ceiling/backpressure 只能把 `parallel` 显式降级为 `single`，不能保留假 parallel 标签。
- **失败恢复**：WorkerPool 不可用/Worker 数不足 → 受控降级或 BLOCKED；不得回到顶层 Master 自己循环工具模拟多 Worker。
- **验证方法**：真实创建两个 READY leaf Task，验证生产 Executor 调用 WorkerPool 并产生 overlap；再限制可用 Worker=1，验证 effective mode 降级且原因可查。
- **Exit**：正式入口的 Multi-Worker 不再依赖 phase script/测试代码手工 `Promise.all`，而是走同一个生产 Dispatch Executor。

### T12.4-B WorkerPool Warm-up / Distinct Role Accounting（v3.5 新增）
- **优先级**：P2
- **需求**：修正 WorkerPool 两个实现级效率/语义问题：互不依赖 Worker 的 async warm-up 可并发；`max_concurrent_roles` 按 distinct Role Profile 数计算，不再用 active worker 数近似。
- **数据来源**：架构文档第 47.8 节。
- **权限**：无新增权限。
- **实现方案**：
  1. `warmAllAsync()` 在 resource ceiling 允许的并行度内并发启动 Worker Instance。
  2. Lease/dispatch 输入携带 `role_profile_ref`（或等价 controller-owned role id）；Coordination Budget 分别记录 `active_workers` 与 distinct `concurrent_roles`。
  3. 同一 researcher Role 的多个 Worker 只计一个 concurrent role，不降低 `max_active_workers` 的独立限制。
- **失败恢复**：role id 缺失 → 保守按每个 Worker 为独立 role 计数，不能低估预算压力。
- **验证方法**：3 个同 Role Worker + 1 个不同 Role Worker 场景验证 role count=2；同时用延迟 lifecycle 证明 warm-up 时间窗口真实重叠。
- **Exit**：Warm-up overlap 与 distinct-role 计数测试通过。
...
### T12.6-A Domain/Provider Backpressure Hook（v3.5 新增）
- **优先级**：P1
- **需求**：网络 Search/Fetch/Browser/HTTP 与 Provider 调用将 `domain/provider/status/retry_after_ms` 结构化上报给统一 backpressure；Master/Worker 不得通过自由生成 shell `sleep` 充当任务调度器。
- **数据来源**：架构文档第 47.5 节；真实 session 中 35 个含 sleep 的工具调用、累计 321 秒等待。
- **权限**：无新增权限；网络权限仍由 Task Contract 控制。
- **实现方案**：
  1. 在通用 Tool/Provider 执行边界提供 backpressure hook，不内置任何单一搜索引擎 API。
  2. 同 domain/provider 的 Retry-After/冷却期进入调度状态；等待 Task 不占用 Master 推理循环，其他 READY Task 可继续。
  3. 有专用 Search/Fetch/Browser 工具时优先使用；退回 Bash/curl 仍必须走 Tool Gateway + Command Risk + backpressure。
  4. Trace 记录 backpressure_wait_ms 与触发来源，供 T11.4-A 评估。
- **失败恢复**：无法解析 Retry-After → 使用有上限的确定性指数 backoff；连续失败仍复用 T12.6 circuit breaker，不允许无限 sleep/retry。
- **验证方法**：模拟同一 domain 返回 429+Retry-After，同时存在另一 domain 的 READY Task，验证前者等待期间后者继续执行，且模型工具调用中没有 `sleep`。
- **Exit**：限流等待从模型行为变为基础设施状态，相关测试中 model-generated sleep=0。
...
### T13.4-A Real User Entry Fan-out/Fan-in Stage Gate（v3.5 新增）
- **优先级**：P0
- **需求**：在 T13.4 专项能力验证之外，再验证一次“用户真实敲 `pph` → Task Compiler → Dispatch Executor → WorkerPool → Receipt/Evidence fan-in”的完整生产路径，防止 Phase 13 能力只存在于测试/phase script 中。
- **数据来源**：架构文档第 47.10 节；T7.0/T7.0-A/T12.4-A。
- **权限**：选择只读 fixture，默认 network/credentials deny，减少非调度因素。
- **实现方案**：
  1. 从正式 bundle 用户入口提交一个可确定性拆成 ≥2 leaf 的多实体只读任务。
  2. 断言 session attestation 表明 ingress/pipeline/scheduler 均绑定。
  3. 断言 persisted Dispatch `requested=parallel/effective=parallel`、effective workers≥2，并有 timestamp overlap ref。
  4. fan-in Task 只获得 MasterHandoffReceipt/Evidence/Artifact refs，不含任一 Worker 完整 transcript marker。
  5. 最终 Verification/Acceptance 仍逐 leaf + fan-in 独立执行。
- **失败恢复**：真实入口任一环没有进入生产 Scheduler → v3.5 Stage Gate FAIL，回退 T7.0-A/T12.4-A 定位，不接受“底层 WorkerPool 单测通过”作为替代证据。
- **验证方法**：真实入口 E2E + 对 session/Persistent State/Trace 三方交叉检查。
- **Exit**：用户入口真实 fan-out/fan-in PASS，证明 Multi-Worker 已从“能力模块”升级为“产品执行路径”。
...
→ T2.0-B（Task Compiler / Ingress Gate，实际最先生效）→ T7.0-A（Execution Surface Attestation）
→ T1.1→T1.1-A→T1.1-B→T1.2→T1.3→T1.3-A→T1.3-B→T1.4→T1.5→T1.6
→ T2.0→T2.0-A→T2.1→T2.2→T2.3→T2.3-A→T2.3-B（Dispatch Execution Contract）→T2.3-C（Independent Work Unit）→T2.4→T2.5→T2.6→T2.7→T2.7-A（Project Registry）→T2.7-B（Task Ledger）→T2.7-C（稳定 CLI 入口）
→ T3.1→T3.2→T3.2-A→T3.2-B（Worker 降级状态+三层模型身份）→T3.3→T3.4→T3.5（含 Fresh Context/Receipt-only 规则）
→ T4.1→T4.1-A（标准化证据包）→T4.1-B（Research Provenance）→T4.2→T4.2-A→T4.2-B→T4.2-C（Tool Gateway）→T4.3→T4.3-A（Acceptance/Commit 分离）→T4.4→T4.5
...
→ T8.x→T8.4-A（resource_ceiling）→T9.x→T10.x（含 T10.3-A Secret-safe Tool Boundary、T10.4 Command Risk Classification、T10.5 Single-Purpose Tool Design）→T11.x（含 T11.4-A/T11.4-B、T11.5/T11.5-A Single-vs-Multi Baseline）
→ T12.0-A（Worker Plugin Manifest 契约，可提前完成）→ T12.1-12.3→T12.4→T12.4-A（Production Dispatch Executor）→T12.4-B（Warm-up/Role Accounting）→T12.5→T12.6→T12.6-A（Domain Backpressure）→T12.7
→ 【Phase 12 Stage Gate PASS】
→ T13.x（Parallel/Batch，含 T13.4 三组强制验收场景 + T13.4-A 真实用户入口 fan-out/fan-in）
...
跑完这一批用 Stage Gate 验收，通过后再开 Phase 2 其余任务（Plan Quality Gate、Role Profile、**Project Registry/Task Ledger/CLI 入口**），依次推进，直到拿到 `PERSONAL_PI_V0.1_CORE` 标签。**Role Profile / Artifact Handoff / Verification Recipe（v2.0）、Work Receipt / Verifier Context Isolation（v3.0）、Phased Permission Escalation / Deterministic Lifecycle Hooks / Scoped Context Assembly（v3.1）、Tool Gateway / 分层预算水位线 / 结构化压缩（v3.3）、Project Registry / Task Ledger / 标准化证据包 / Acceptance-Commit 分离（v3.4）、Dispatch Execution Contract / Execution Surface Attestation / Production Dispatch Executor / Secret-safe Tool Boundary / Real-entry Multi-Worker Gate（v3.5）均已按现有 Phase 边界插入，不新增平行开发主线。**

**v3.5 新增纪律（源自 2026-09-19 真实多实体研究 session 复盘）：**
14. **Dispatch Policy 不是执行事实**——`requested_mode` 与 `effective_mode` 必须分开；没有 ≥2 Worker + overlap 证据就不能把实际执行记成 PARALLEL。
15. **任何真实用户 session 必须留下 Execution Surface Attestation**——entrypoint/bundle/commit/Ingress/Pipeline/Scheduler 是否绑定必须可审计，不能靠事后猜测。
16. **等待/限流属于 backpressure，不属于 Master 的 shell 策略**——禁止让模型用自由生成的 `sleep` 承担 provider/domain 排队。
17. **多实体研究先拆 independent units，再 fan-out，再 Receipt/Evidence fan-in**——Master 不继承 Worker 搜索 transcript；关键数字必须带 provenance。
18. **`credentials=deny` 同时约束输入与工具输出**——环境全量枚举默认阻断，secret-like 键值必须在任何模型可见摘要前脱敏。
...
| **v3.5** | **2026-09-19** | **用户+ChatGPT 评审确认** | **流程变更 + 新增任务（真实 session 驱动）** | **Phase 2/4/7/10/11/12/13 + 文件管理方式** | **保留“latest 持续更新 + 实现前完整快照”双轨流程；新增 T7.0-A、T2.3-B/C、T4.1-B、T10.3-A、T11.5-A、T12.4-A/B、T12.6-A、T13.4-A，分别落地 Execution Surface Attestation、Dispatch 真值契约、Independent Work Unit、Research Provenance、Secret-safe Tool Boundary、真实 research benchmark、Production Dispatch Executor、WorkerPool warm-up/Role 计数、domain backpressure 与真实用户入口 fan-out/fan-in Stage Gate。** |
