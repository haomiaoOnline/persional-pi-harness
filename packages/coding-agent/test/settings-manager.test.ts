...
describe("SettingsManager", () => {
	...
	describe("project settings directory creation", () => {
		it(`should not create ${CONFIG_DIR_NAME} folder when only reading project settings`, () => {
			...
			// The project config folder should NOT have been created just from reading
		...
		it(`should create ${CONFIG_DIR_NAME} folder when writing project settings`, async () => {
			// Create agent dir with global settings, but NO project config folder
			const settingsPath = join(agentDir, "settings.json");
			writeFileSync(settingsPath, JSON.stringify({ theme: "dark" }));
